...
class WNO2d(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.reshape(x.shape[0],32,32,1).float()
...
class InverseQuadraticKernel(gpytorch.kernels.Kernel):
    ...
    def forward(self, x1, x2, **params):
        covar_matrix = self.base_kernel(x1, x2, **params)
        return 1 / covar_matrix.sqrt()
...
class IQKernel(Kernel):
    ...
    def __init__(self, **kwargs):
        super(IQKernel, self).__init__(**kwargs)
  

    def forward(self, x1, x2, diag=False, **params):
        # Compute squared distances
        dist_mat = self.covar_dist(x1, x2, square_dist=True, diag=diag, **params)

        if diag:
            # Return diagonal elements
            return torch.ones_like(dist_mat.diagonal())

        # Compute inverse quadratic kernel
        unitless_sq_dist = dist_mat.div(self.lengthscale).pow(2)
        return 1 / torch.sqrt(unitless_sq_dist + 1)
