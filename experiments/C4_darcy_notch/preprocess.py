...
def load_and_preprocess(config: Config) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, UnitGaussianNormalizer, UnitGaussianNormalizer, torch.Tensor]:
    ...
    Returns:
        ...
        x_t: Testing inputs
        y_t: Testing targets
        x_normalizer: Normalizer for inputs
        y_normalizer: Normalizer for outputs
        x_train_original: Original training inputs with channel dimension
    """
    # Load data
    reader_input = np.load(config.train_path)  # shape: (64, 64, 40000)
    reader_output = np.load(config.test_path)  # shape: (64, 64, 40000)
    
    # Convert to torch tensors and permute
    ...
    # For dummy_data, need x_train before reshaping, with channel dimension
